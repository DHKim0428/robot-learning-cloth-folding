...
```
PAUSED  ──[Space]──►  AUTONOMOUS  ──[Tab]──►  CORRECTING
  ▲                                                  │
  └──────────────────────────────[Tab]───────────────┘
```
...
|-----|-----------|--------|
| `Space` | PAUSED | Start/resume autonomous rollout |
| `Tab` | AUTONOMOUS → CORRECTING | Take over directly and start correction |
| `Tab` | CORRECTING → PAUSED | Stop the correction and return to PAUSED |
| `→` right arrow | not CORRECTING | Save the full rollout only if it had an intervention, reset, and wait at start pose |
| `←` left arrow | any | Discard the current rollout, reset, and wait at start pose |
