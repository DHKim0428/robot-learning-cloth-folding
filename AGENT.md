...
The final course-compliant policy must be a single model that completes the whole task. Do not design or document separate grasp/fold policies or a handoff-based final method.
...
- Then shift to one compliant diffusion policy model that handles grasping, single fold, and double fold.
...
- Hybrid final systems with multiple learned policies are not allowed; the final task must be solved by one model.
