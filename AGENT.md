...
- `config/two_stage_episode_manifest.toml` - append-only stage/quality manifest for two-stage episodes; update with `scripts/update_two_stage_episode_manifest.py`.
...
`update_two_stage_episode_manifest.py` counts actual LeRobot episode metadata rows and appends only missing entries to `config/two_stage_episode_manifest.toml`. It never rewrites existing episode values. `--count` prints total, stage-1, and stage-2 counts and does not read or write the manifest.
