"""LeRobotDataset wrapper for single-policy diffusion training.

The default path intentionally relies on the Hugging Face dataset repo/cache
instead of this repository's local `data/` folder. Passing `--dataset-root` is
still supported for explicit debugging or pinned local datasets.
"""
...
HF_DATASET_ACCESS_HINT = (
    "If this is a private Hugging Face dataset, authenticate first with "
...
def _root_kwargs(dataset_root: Path | None) -> dict[str, Path]:
    return {} if dataset_root is None else {"root": dataset_root}


def load_metadata(repo_id: str, dataset_root: Path | None) -> LeRobotDatasetMetadata:
    """Load metadata from the HF cache/download path unless `dataset_root` is set."""
    return LeRobotDatasetMetadata(repo_id, **_root_kwargs(dataset_root))


def load_episode_filter(mode: str | None) -> set[int]:
    """Resolve the project-level quality filter."""
...
def build_delta_timestamps(cfg, fps: int) -> dict[str, list[float]]:
    """Build LeRobot delta timestamps for diffusion observations and actions."""
    obs_delta = _make_delta_timestamps(cfg.observation_delta_indices, fps)
...
def select_episodes(
    ...
    """Resolve episode indices to load.

    The filter is quality-only; it does not split the task into stages.
    """
    ...
    return [idx for idx in range(meta.total_episodes) if idx not in ignored]
...
def _sanitize_loaded_rows(dataset: LeRobotDataset) -> None:
    ...
    if not required_columns.issubset(set(hf_dataset.column_names)):
        return
    ...
    print(
        f"[warn] dropped {removed} stale/invalid rows from dataset cache view "
...
def build_dataset(
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to load dataset {repo_id!r}.\n{HF_DATASET_ACCESS_HINT}"
        ) from exc
