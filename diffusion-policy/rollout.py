...
Mirrors ``ACT_model/rollout.py`` but rebuilds a ``DiffusionPolicy``
(see ``build_policy.build_diffusion_policy``) before loading the
checkpoint, so the backbone / horizon / chunking args here must match
those used at training time.

Usage:
    bash shell/rollout_diffusion.sh diffusion-policy/outputs/<run>/policy_diffusion.pt
or:
    python diffusion-policy/rollout.py --checkpoint <path> [--dry-run]
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a trained Diffusion Policy on the SO-101 follower arm."
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
        required=True,
        help="Path to policy_diffusion.pt (a state_dict produced by diffusion-policy/train.py).",
    )
    parser.add_argument(
        "--dataset-repo-id",
...
def _build_observation(
    ...
    """Convert SO-101 raw observation -> diffusion-style batched tensor dict.

    The dataset stores ``observation.state`` (6-vec) and
    ``observation.images.front`` (HWC uint8). The robot returns
    ``{f"{motor}.pos": float, "front": ndarray}``.
    """
    state_vec = np.array(
...
def main() -> None:
    ...
    policy.eval()
    policy.reset()
    print(f"[init] loaded DiffusionPolicy from {args.checkpoint}")
